...
const authlessFull = computed(() => route.meta.authlessFull === true)
...
<template>
  <div class="app-shell" :class="{ 'app-shell--authless': !showShell }">
    ...
    <main
      v-else
      class="authless-panel"
      :class="{ 'authless-panel--wide': authlessWide, 'authless-panel--full': authlessFull }"
    >
      <RouterView />
    </main>
  </div>
...
.mobile-header {
  ...
  gap: 1rem;
  padding: 0.9rem 1rem;
  background: rgba(232, 225, 213, 0.9);
