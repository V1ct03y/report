...
test('管理员结算后还需要额外确认公示', async ({ browser }) => {
  ...
  const settleButton = adminPage.getByRole('button').filter({ hasText: /手动结算|执行结算/ }).first()
  if (await settleButton.isVisible().catch(() => false)) {
    await settleButton.click()
    await adminPage.waitForTimeout(1500)
  }

  const publishButton = adminPage.getByRole('button', { name: '确认公示' })
  ...
  await expect(
    adminPage.getByRole('button').filter({ hasText: /归档公示|手动同步|手动结算|执行结算/ }).first()
  ).toBeVisible()
